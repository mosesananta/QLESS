...
def collect_grads(dataloader,
    ...
    save_interval = max(int(len(dataloader)*0.1), 160) # save every 1/10 of dataloader processed
    ...
    def _save(projected_grads, output_dirs):
        for dim in proj_dim:
            ...
            # Ensure precision
            projected_grads[dim] = projected_grads[dim].to(torch.float16)
...
def merge_and_normalize_info(output_dir: str, prefix="reps"):
    ...
    for file in info:
        ...
        merged_data.append(normalized_data.to(torch.float16))  # Ensure precision
...
def merge_info(output_dir: str, prefix="reps"):
    ...
    for file in info:
        data = torch.load(os.path.join(output_dir, file))
        merged_data.append(data.to(torch.float16))  # Ensure precision
    merged_data = torch.cat(merged_data, dim=0)
